...
step_gpu = ElementwiseKernel("unsigned int *fld, unsigned int *fld_new, unsigned int *seeds, unsigned int *img, int w, int h", """
    ...
    if (energy >= 0xff || f0 > 0 && (((f0 >> 8) & (1 << N)) == 0)) {
        ...
        for (int ni = 8; ni > 0; ni--) {
            ...
            if (ff1 + ff2 + ff3 + ff4 + ff5 + ff6 + ff7 + ff8 == ni) {
                ...
                while (gene_num < 17) {
                    ...
                    if (seed > 0) seed--;
                    if (seed == 0 && ff8 && gene_num < 17) {
                        gene = (1 << (gene_num + seed) % 17) & f8;
                        f0 += gene;
                        gene_num++;
                        if (f00 == 0 && gene > 0) fld_new[xp1 * h + yp1] += ({1} << 17);
                        if (gene > 0 && gene > 0) genes_count--;
                        if (genes_count <= 0) break;
                    }
                    if (seed > 0) seed--;
                }
        ...
        fld_new[i] = f0;
        energy = (f0 >> 17);
        if (energy > 0xff) energy = 0xff;
        img[i] = (f0 == 0 ? fadeout(img0, 5) : hsv2rgb((f0 & 0x1ffff) % 360, 0xff - energy, 255));
...
class EvoLife:
    ...
    def __init__(self, width=0, height=0, fullscreen=False, saveframes=False, downscale_factor=1, frame_skip=1):
        ...
        seeds = np.asarray([[random.randint(0, 16) for j in range(self.height)] for i in range(self.width)]).astype(np.int32)
        fld = fld_init(self)
    ...
    def step(self):
        ...
        step_gpu(self.f1_gpu, self.f2_gpu, self.seeds_gpu, self.img_gpu, np.uint32(self.width), np.uint32(self.height))
        tmp = self.f1_gpu
